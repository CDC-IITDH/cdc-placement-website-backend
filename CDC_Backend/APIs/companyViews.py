...
@precheck([COMPANY_NAME, ADDRESS, COMPANY_TYPE, NATURE_OF_BUSINESS, WEBSITE, COMPANY_DETAILS, IS_COMPANY_DETAILS_PDF,
           CONTACT_PERSON_NAME, PHONE_NUMBER, EMAIL, CITY, STATE, COUNTRY, PINCODE, DESIGNATION, DESCRIPTION, IS_DESCRIPTION_PDF,
           COMPENSATION_CTC, COMPENSATION_GROSS, COMPENSATION_TAKE_HOME, COMPENSATION_BONUS, COMPENSATION_DETAILS, IS_COMPENSATION_DETAILS_PDF,
           ALLOWED_BRANCH, SELECTION_PROCEDURE_ROUNDS, SELECTION_PROCEDURE_DETAILS, IS_SELECTION_PROCEDURE_DETAILS_PDF, TENTATIVE_DATE_OF_JOINING,
    ...
    try:
        ...
        if opening.is_company_details_pdf:
            ...
            for file in files.getlist(COMPANY_DETAILS_PDF):
                path = STORAGE_DESTINATION_COMPANY_ATTACHMENTS + opening.id + '/'
                company_details_pdf.append(saveFile(file, path))
        ...
        if opening.is_description_pdf:
            ...
            for file in files.getlist(DESCRIPTION_PDF):
                path = STORAGE_DESTINATION_COMPANY_ATTACHMENTS + opening.id + '/'
                description_pdf.append(saveFile(file, path))
        ...
        if opening.is_compensation_details_pdf:
            ...
            for file in files.getlist(COMPENSATION_DETAILS_PDF):
                path = STORAGE_DESTINATION_COMPANY_ATTACHMENTS + opening.id + '/'
                compensation_details_pdf.append(saveFile(file, path))
        ...
        if opening.is_selection_procedure_details_pdf:
            ...
            for file in files.getlist(SELECTION_PROCEDURE_DETAILS_PDF):
                path = STORAGE_DESTINATION_COMPANY_ATTACHMENTS + opening.id + '/'
                selection_procedure_details_pdf.append(saveFile(file, path))
